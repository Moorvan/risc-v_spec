...
class Memory:
    ...
    def get_mem(self, addr: int, funct3: int):
        res = Bit_Utils.bit_slice(self._data[addr], 7, 0)  # LB or LBU, H, W, D
        w = 8
        if funct3 % 4 >= 1:  # LH or LHU, W, D
            res += Bit_Utils.bit_slice(self._data[addr + 1], 7, 0) << 8
            w += 8
        if funct3 % 4 >= 2:  # LW or LWU, D
            res += Bit_Utils.bit_slice(self._data[addr + 2], 7, 0) << (8 * 2)
            res += Bit_Utils.bit_slice(self._data[addr + 3], 7, 0) << (8 * 3)
            w += 16
        if funct3 % 4 >= 3:  # LD or LDU
            res += Bit_Utils.bit_slice(self._data[addr + 4], 7, 0) << (8 * 4)
            res += Bit_Utils.bit_slice(self._data[addr + 5], 7, 0) << (8 * 5)
            res += Bit_Utils.bit_slice(self._data[addr + 6], 7, 0) << (8 * 6)
            res += Bit_Utils.bit_slice(self._data[addr + 7], 7, 0) << (8 * 7)
            w += 32
